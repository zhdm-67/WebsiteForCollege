...
const items = [
  {id:1, name:'Латте', cat:'coffee', price:5.69, desc:'Молоко, эспрессо, бархатная пена', img:'img/coffee.jpg'},
  {id:2, name:'Капучино', cat:'coffee', price:5.99, desc:'Классический капучино с корицей', img:'img/coffee.jpg'},
  {id:3, name:'Круассан', cat:'bakery', price:4.2, desc:'С маслом, хрустящий', img:'img/croissant.jpg'},
  {id:4, name:'Булочка с корицей', cat:'bakery', price:6.7, desc:'Нежная и ароматная', img:'img/cinnamon.jpg'},
  {id:5, name:'Лимонад', cat:'cold', price:4.99, desc:'Освежающий домашний лимонад', img:'img/lemonade.jpg'},
  {id:6, name:'Холодный кофе', cat:'cold', price:5.49, desc:'Айс латте с ванилью', img:'img/coffee.jpg'}
];
...
function renderMenu(filter='all'){
  ...
  if(!container) return;
  container.innerHTML = '';
  const filtered = items.filter(i => filter==='all' ? true : i.cat===filter);
  filtered.forEach(it=>{
...
function getCartElements(suffix=''){
  return {
    cartBtn: document.getElementById(`cartBtn${suffix}`) || document.getElementById('cartBtn'),
    cartCount: document.getElementById(`cartCount${suffix}`) || document.getElementById('cartCount'),
    modalBackdrop: document.getElementById(`modalBackdrop${suffix}`) || document.getElementById('modalBackdrop'),
    cartList: document.getElementById(`cartList${suffix}`) || document.getElementById('cartList'),
    cartTotal: document.getElementById(`cartTotal${suffix}`) || document.getElementById('cartTotal'),
    closeModal: document.getElementById(`closeModal${suffix}`) || document.getElementById('closeModal'),
    clearCart: document.getElementById(`clearCart${suffix}`) || document.getElementById('clearCart'),
    checkout: document.getElementById(`checkout${suffix}`) || document.getElementById('checkout'),
  };
}
...
function updateCartUI(suffix=''){
  ...
  Array.from(els.cartList.querySelectorAll('.inc')).forEach(b=>{
    b.addEventListener('click', e=>{
      ...
      cart[id] = (cart[id]||0)+1; saveCart(); updateAllCartCounts();
    });
  });
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    document.body.addEventListener('click', e=>{
        ...
        document.querySelectorAll('.cart-float').forEach(b=>{
            b.animate([{transform:'scale(1)'},{transform:'scale(1.08)'},{transform:'scale(1)'}],{duration:320,easing:'ease-out'});
        });
        updateAllCartCounts();
        }
    });
    ...
    [['',''],['2','2']].forEach(([suf,s])=>{
        ...
        setTimeout(()=> els.modalBackdrop.querySelector('.modal')?.classList.add('open'),20);
        updateCartUI(suf);
        });
        els.closeModal?.addEventListener('click', ()=>{
        if(!els.modalBackdrop) return;
        els.modalBackdrop.querySelector('.modal')?.classList.remove('open');
        setTimeout(()=>{ els.modalBackdrop.style.display='none'; els.modalBackdrop.setAttribute('aria-hidden','true'); },240);
        });
        ...
        cart = {}; saveCart(); updateAllCartCounts();
        });
    ...
    document.querySelectorAll('[id^="orderNow"]').forEach(btn=>{
        btn.addEventListener('click', ()=> document.querySelector('.cart-float')?.click());
    });
    ...
    if(contactFormEl){
        contactFormEl.addEventListener('submit', e=>{
            ...
            if(!valid){
                ...
                contactFormEl.animate([{transform:'translateY(0)'},{transform:'translateY(-6px)'},{transform:'translateY(0)'}],{duration:320});
                return;
            }
            ...
            setTimeout(()=>{
                ...
                setTimeout(()=>{ toast.classList.remove('show'); setTimeout(()=> toast.hidden = true,320); }, 3000);
            }, 900);
        });
